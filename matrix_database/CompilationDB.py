...
def SaveCompilationResults2DB(builds, dbname):
    ...
    try:
        ...
        print("ERROR: failed to insert compilation results to database", dbname, sys.exc_info()[0], sys.exc_info()[1])
        sys.exit(1)
...
class CompilationDB:
    ...
    def FillTables(self, builds):
        ...
        for m in range(0, len(builds)):
            dbfile_load_status, build_instance_id = self.BuildLogLoaded(
                builds[m].name, builds[m].raw_file)
            if dbfile_load_status == 1:
                ...
                self.AddBuildInstance(
                    builds[m], dbfile_load_status, build_instance_id)
                if m == 0:
    ...
    def AddBuild(self, build):
        ...
        if build_id != 0:
            ...
            if (name in BuildConfig.config) == 0:
                query = "INSERT INTO build VALUES (NULL, '%s', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);" % (
                    name)
            else:
                config = BuildConfig.config[name]
                query = "INSERT INTO build VALUES (NULL, '%s', '%s', '%s', %d, '%s', %d, %d, %d, %d);" % (
                    name, config[0], config[1], config[2], config[3], config[4], config[5], config[6], config[7])
    ...
    def AddBuildInstance(self, build, dbfile_load_status, build_instance_id
        ...
        if (build.name in self.build_ids) == 0:
            self.AddBuild(build.name)
        if dbfile_load_status == -1:
            query = "INSERT INTO build_instance VALUES (NULL, %d, '%s', '%s', NULL, '%s', NULL, NOW());" % (self.build_ids[build.name], str(
                Parser.DateTimeFromString(build.start_time)), str(Parser.DateTimeFromString(build.end_time)), build.raw_file)
            self.curs.execute(query)
    ...
    def AddCompilationInstance(self, build):
        for n in range(0, len(build.compilation_results)):
            if (
                    build.compilation_results[n].name in self.project_ids) == 0:
                self.AddProject(build.compilation_results[n].name)
            if build.compilation_results[n].skipped == 1:
                query = "INSERT INTO compilation_instance VALUES(%d, %d, 1, NULL, NULL);" % (
                    self.project_ids[build.compilation_results[n].name], self.build_instance_ids[build.name])
            else:
                query = "INSERT INTO compilation_instance VALUES(%d, %d, 0, %d, %d);" % (
                    self.project_ids[build.compilation_results[n].name], self.build_instance_ids[build.name], build.compilation_results[n].num_errors, build.compilation_results[n].num_warnings)
            # print "AddCompilationInstance ", query
            try:
                self.curs.execute(query)
            except _mysql_exceptions.IntegrityError:
                print("AddCompilationInstance failed: ", build.compilation_results[n].name, build.raw_file, sys.exc_info()[0], sys.exc_info()[1])

    def BuildLogLoaded(self, build_name, log_fname):
        dbfile_load_status = -1
        build_instance_id = -1
        query = "SELECT build_instance_id, compilation_insert_time FROM build_instance WHERE log_fname='%s';" % (
            log_fname)
        ...
        # print  "BuildLogLoaded ", query, "build_instance_id=",
        # build_instance_id, "compilation_loaded=", dbfile_load_status
        return dbfile_load_status, build_instance_id

    def GetBuildId(self, build_name):
        build_id = 0
        query = "SELECT build_id FROM build WHERE build_name='%s';" % (
            build_name)
