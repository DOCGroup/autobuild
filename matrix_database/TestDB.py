...
def SaveTestResults2DB(builds, dbname):
    ...
    try:
        ...
        sys.exit('ERROR: failed to insert test results to database' + ' '.join([
            dbname, sys.exc_info()[0], sys.exc_info()[1]]))
...
class TestDB:
    ...
    def AddBuildInstance(self, build, dbfile_load_status, build_instance_id):
        ...
        if dbfile_load_status == -1:
            query = "INSERT INTO build_instance VALUES (NULL, %d, '%s', '%s', NULL, '%s', NOW(), NULL);" % (self.build_ids[build.name], str(
    ...
    def AddTestInstance(self, build):
        for n in range(0, len(build.test_results)):
            if (build.test_results[n].passFlag == PASS):
                ...
                pass_flag = 'S'
            if (build.test_results[n].name in self.test_ids) == 0:
                self.AddTest(build.test_results[n].name)
            query = "INSERT INTO test_instance VALUES(%d, %d, '%s', '%s');" % (
                self.test_ids[build.test_results[n].name], self.build_instance_ids[build.name], pass_flag, build.test_results[n].time)
            # print "AddTestInstance ", query
            try:
                self.curs.execute(query)
            except _mysql_exceptions.IntegrityError:
                print("AddTestInstance failed: ", build.test_results[n].name, build.raw_file, sys.exc_info()[0], sys.exc_info()[1])

    def BuildLogLoaded(self, build_name, log_fname):
        db_load_status = -1
        build_instance_id = -1
        query = "SELECT build_instance_id, test_insert_time FROM build_instance WHERE log_fname='%s';" % (
            log_fname)
        ...
        # build_instance_id, "test_loaded=", db_load_status
