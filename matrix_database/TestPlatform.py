...
class TestMatrix:
    def __init__(self, num_platforms):
        ...
        self.testResults = [
            [0] *
            num_platforms for row in range(
                self.max_tests)]
...
class TestPlatform:
    ...
    def writeDBLog(self):
        ...
        for n in range(0, len(self.test_results)):
            test_str = self.test_results[n].name + ";" + str(
                self.test_results[n].passFlag) + ";" + str(self.test_results[n].time)
            fh.write(test_str + "\n")
        fh.close()
    ...
    def processDBLog(self):
        ...
        # print "processDBLog ", self.db_file, self.name, self.raw_file,
        # self.start_time, self.end_time, self.compile
        parse_error = 0
        for line in fh:
            line = line.strip()
            splits = line.split(";")
            if len(splits) != 3 or splits[0] == "":
                ...
                break
            name = splits[0]
            passflag = int(splits[1])
            time = float(splits[2].strip())
            ...
            if (parse_error == 1 or self.name == "" or self.raw_file == "" or self.start_time ==
                    "" or self.end_time == "" or len(self.test_results) == 0):
    ...
    def scanForTestFailure(self, fh, stop_strings):
        ...
        for line in fh:
            line = line.strip()
            if any([line.startswith(s) for s in stop_strings]):
                break
            elif test_error.search(line):
                fail = 1
            elif test_finished.match(line):
                testname, resultCode, time = self.checkTestTime(line)
            elif build_end.match(line):
                m = line.find('[')
                n = line.find('UTC')
    ...
    def processLog(self):
        ...
        time = 0.0
        # scan thru the file, state=0 while in pre-test stuff and state=1 while
        # in tests
        while line:
            readline = 0
            ...
            if state == 1 and test_start.match(line):
                ...
                line, fail, time, testname = self.scanForTestFailure(
                    file, stop_strings)
                if testname == "":
